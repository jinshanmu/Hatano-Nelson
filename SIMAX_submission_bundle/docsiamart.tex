\documentclass[final,onefignum,onetabnum]{siamart251216}
...
at \url{https://epubs.siam.org/pb-assets/files/SIAM_STYLE_GUIDE_2019-1635349464967.pdf}.
%
...
\begin{itemize}
\item \texttt{siamart251216.cls} (required): Main SIAM standard \LaTeX\ class file.
...
\end{itemize}
To use these files, put \texttt{siamart251216.cls} and
...
aside from the ones listed in the \href{https://epubs.siam.org/pb-assets/files/SIAM_STYLE_GUIDE_2019-1635349464967.pdf}{SIAM Style Manual (Chapter 6)} will need to use \code{\ref}
instead of \code{\cref} when referenced in the document.  The theorem-like
...
\texttt{consequence}, and
\texttt{problem} 
can also be defined as a remark-like environment in accordance with the \href{https://epubs.siam.org/pb-assets/files/SIAM_STYLE_GUIDE_2019-1635349464967.pdf}{SIAM Style Manual (Chapter 6)} without having an issue with \texttt{cleveref} if an author chooses to do so.
...
\Cref{ex:simpletable} shows the code to generate
...
\documentclass[supplement]{siamart251216}
\input{ex_shared}
...
\begin{itemize}
\item Change in file name: \texttt{siamart251104.cls} is replaced by \texttt{siamart251216.cls}.
\item Additional margin adjustments for print vendors.
\item Adjusted document trim to match print publication trim of 6.75" x 10".
\item Added a fix for \texttt{cleveref} referencing anything defined as a theorem-like environment as a theorem.
\item Added a fix for \code{\label} and \code{\refstepcounter} due to \texttt{hyperref} and \texttt{cleveref} package changes.
...
\cref{ex:numbering} as documentclass options (e.g., \code{\documentclass[review,onefignum,onetabnum]{siamart251216}}).
