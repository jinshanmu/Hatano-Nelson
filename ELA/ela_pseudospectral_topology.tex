...
\newcommand{\Title}{Connectedness thresholds for complex tridiagonal Toeplitz
pseudospectra: monotonicity with matrix order}

\hypersetup{
  pdftitle={\Title},
  pdfauthor={Shanmu Jin},
  pdfsubject={Connectedness thresholds for complex tridiagonal Toeplitz pseudospectra},
...
\markboth{\Names}{Connectedness thresholds for complex tridiagonal Toeplitz
pseudospectra}
\maketitle
...
\(T_n(\alpha,d,\beta)=\tridiag(\alpha,d,\beta)\), \(n\geq2\), is determined
for all \(\alpha,d,\beta\in\C\).  Set
\(c=\max\{|\alpha|,|\beta|\}\).  For \(c>0\), unitary similarity and affine
transformations reduce the problem to
\(A_n(a)=\tridiag(a,0,1)\), where
...
eigenvalues, the first coalescence occurs at a generalized saddle point of
the least-singular-value function, whose level determines the Euclidean-norm
distance to defectivity \cite{AlamEtAl2011}.  This leads to a
finite-dimensional topological problem not determined by the spectrum alone.
...
Euclidean-norm pseudospectral geometry has only two effective real parameters.
When
\[
 c=\max\{|\alpha|,|\beta|\}>0,
...
The finite-order difficulty is to compare global barriers on spectral
intervals that change with the matrix order.  Diagonal similarity identifies
the spectrum, but its condition number in the Euclidean operator norm
\(a^{-(n-1)/2}\) gives no control of the corresponding least-singular-value
geometry.  Interlacing is pointwise, whereas
the barrier maximizes over moving gaps whose central geometry changes with
parity.  Toeplitz asymptotics determine an eventual scale but do not imply a
strict inequality at every pair of adjacent orders.

The relevant literature spans Toeplitz spectral theory and pseudospectral
optimization.  Spectral limit sets of banded Toeplitz matrices go back to
Schmidt and
Spitzer \cite{SchmidtSpitzer1960}; finite Toeplitz spectra, inverse norms, and
pseudospectra are studied in
\cite{ReichelTrefethen1992,BottcherGrudsky2005}, and tridiagonal Toeplitz
structure is treated in \cite{NoschesePasquiniReichel2013}.  Pseudospectral
optimization, component coalescence, and distance to defectivity are studied
in \cite{BurkeLewisOverton2003,AlamEtAl2011}.  Recent work on tridiagonal
\(k\)-Toeplitz matrices treats broader periodic coefficient patterns
\cite{AmmariEtAl2025,AmmariCorrigendum2026}.  Numerical studies of
exponentially sensitive lattices have also visualized order-driven merger of
pseudospectral clouds in Hatano--Nelson examples
\cite{KiorpelidisMakris2025}, while recent analysis discusses the associated
pseudospectral instability, its exponential condition-number scale, and the
finite-chain residual cutoff \cite{Sirker2026}.
The contribution developed here is the all-order finite-dimensional
inequality that the maximizing least-singular-value barrier decreases
strictly between consecutive matrix orders.

For the canonical nonnormal regime \(0<a<1\), write
\[
...
\end{equation}
The factor \(n+1\) in \eqref{eq:intro-barrier-order} is optimal.  Consequently,
connectedness persists at larger orders, and the first connected order has a
two-term logarithmic asymptotic.  The unitary--affine reduction transfers
these conclusions to the full complex family.  At \(a=1\), the
barrier is half the largest adjacent eigenvalue gap and decreases strictly at
order \((n+1)^{-1}\); at \(a=0\), the pseudospectrum is a disk and is
connected for every \(n\).  Thus the nonnormal interior, normal boundary, and
one-sided boundary have exponential--polynomial, algebraic, and zero
threshold scales, respectively.

A complementary result applies beyond Toeplitz matrices.  For a finite matrix
with real spectrum, contraction of least-singular-value sublevel sets toward
the real axis makes every pseudospectral component contractible and reduces
connectedness to the largest backward-error barrier on the spectral interval.
Rotation and scaling extend the result to spectra on any affine line.
...
\end{aligned}
\end{equation}
where \(P_m(x)=xI_m-A_m\).  In \eqref{eq:even-dilation},
...
The three threshold scales show that both boundary regimes are singular
limits of the nonnormal interior.  For \(0<a<1\), the threshold has the
exponential--polynomial scale \(c(n+1)a^{n/2}\); at \(a=1\), it has the
algebraic scale \(\pi c/(n+1)\); and at \(a=0\), it vanishes.  Accordingly,
the remainder \(O_a(1)\) in \eqref{eq:N-asymptotic-main} is pointwise for
fixed \(a\) and is not uniform at either boundary.

Strict matrix-order monotonicity makes the connected orders an integer tail
for every irreducible family.  At the reducible boundary, this tail begins at
\(n=2\) because the barrier vanishes identically.  Thus
\(N_T(\alpha,d,\beta;\varepsilon)\) marks the first order at which all
pseudospectral clusters have coalesced.
...
The pseudospectrum here uses the Euclidean vector norm, equivalently the full
ball of complex perturbations in the induced operator norm.  Toeplitz,
tridiagonal, real, or entrywise perturbation constraints change the attainable
set and may alter the connectedness threshold; see
\cite{ButtaGuglielmiNoschese2012} for Toeplitz-structured pseudospectra.
Structured perturbations, block Toeplitz matrices, and periodic
\(k\)-Toeplitz families are natural settings for extending the order
comparison.
...
exponent appears in \(a^{n/2}=e^{-hn}\).  Thus eigenvector localization and
pseudospectral connectedness share the same exponential scale in the
nonreciprocal chain.
