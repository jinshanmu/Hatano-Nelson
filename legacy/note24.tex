...
\section{Connectivity thresholds and the real-axis barrier}\label{sec:thresholds}

In this section we collect the additional results obtained in the study of the
connectedness threshold.  Throughout, \(0<a<1\) is fixed and
\[
\mathbf A_n(a)=\operatorname{tridiag}(a,0,1),\qquad n\ge2.
\]
We write
\[
\Sigma_{\varepsilon,n}:=\sigma_\varepsilon\bigl(\mathbf A_n(a)\bigr),
\qquad
s_n(x):=s_{\min}(xI_n-\mathbf A_n(a)),\qquad x\in\mathbb R,
\]
and denote the eigenvalues of \(\mathbf A_n(a)\) by
\[
\lambda_k^{(n)}=2\sqrt a\cos\frac{k\pi}{n+1},\qquad k=1,\dots,n.
\]

For \(1\le j\le q_n:=\lfloor n/2\rfloor\), let \(G_{n,j}\) be the \(j\)-th positive
real gap counted from the upper edge:
\[
G_{n,j}:=
\begin{cases}
[\lambda_{j+1}^{(n)},\lambda_j^{(n)}],& 1\le j\le q_n-1,\\[1mm]
[0,\lambda_{q_n}^{(n)}],& j=q_n\ \text{and }n\ \text{odd},\\[1mm]
[\lambda_{q_n+1}^{(n)},\lambda_{q_n}^{(n)}],& j=q_n\ \text{and }n\ \text{even}.
\end{cases}
\]
Define the corresponding real-axis barriers by
\[
\beta_{n,j}(a):=\max_{x\in G_{n,j}} s_n(x),
\qquad
\beta_n(a):=\max_{1\le j\le q_n}\beta_{n,j}(a).
\]
Finally, for \(\varepsilon>0\) set
\[
N_c^{\mathrm{first}}(a,\varepsilon)
:=
\min\{n\ge2:\Sigma_{\varepsilon,n}\ \text{is connected}\},
\]
\[
N_c^{\mathrm{ev}}(a,\varepsilon)
:=
\min\{N\ge2:\Sigma_{\varepsilon,n}\ \text{is connected for every }n\ge N\}.
\]

\begin{proposition}[Barrier criterion for connectedness]
\label{prop:barrier-criterion}
For every \(n\ge2\) and \(\varepsilon>0\),
\[
\Sigma_{\varepsilon,n}\ \text{is connected}
\iff
\beta_n(a)<\varepsilon.
\]
Equivalently, the connectedness of \(\Sigma_{\varepsilon,n}\) is completely governed by
the maximum of the least singular value on the real spectral gaps.
In particular, by Theorem~\ref{thm:main}, connectedness is equivalent to
simple connectedness.
\end{proposition}

\begin{proof}
By Proposition~\ref{prop:topological-reduction}, every connected component of
\(\Sigma_{\varepsilon,n}\) is vertically convex and is determined by its intersection
with the real axis. By Lemma~\ref{lem:real-axis}, each connected component meets
\(\mathbb R\). Hence a real gap \(G_{n,j}\) separates two neighboring components
precisely when
\[
\max_{x\in G_{n,j}}s_n(x)\ge\varepsilon.
\]
Therefore no separation occurs if and only if
\[
\max_{1\le j\le q_n}\max_{x\in G_{n,j}}s_n(x)=\beta_n(a)<\varepsilon,
\]
which is the stated criterion.
\end{proof}

\begin{proposition}[Elementary bounds]
\label{prop:beta-rough}
For every \(n\ge2\),
\[
\frac{2a^{n/2}}{n+1}\le \beta_n(a)\le (n+1)a^{n/2}.
\]
In particular, \(\beta_n(a)\to0\) exponentially as \(n\to\infty\).
\end{proposition}

\begin{theorem}[Asymptotics of the fixed-gap barriers]
\label{thm:fixed-gap-barriers}
For each fixed \(j\ge1\),
\[
\beta_{n,j}(a)
=
\gamma_j\,\frac{(1-a)^3}{1+a}\,(n+1)\,a^{n/2}
+
O_{a,j}\!\left(\frac{a^{n/2}}{n}\right),
\qquad n\to\infty,
\]
where
\[
\gamma_j:=\max_{y\in(j\pi,(j+1)\pi)}\frac{|\sin y|}{y}.
\]
Moreover,
\[
\gamma_1>\gamma_2>\gamma_3>\cdots,
\]
and the central gap satisfies only
\[
\beta_{n,q_n}(a)=O(a^{n/2}).
\]
Consequently, for all sufficiently large \(n\),
\[
\beta_n(a)=\beta_{n,1}(a),
\]
that is, the outermost real gap is asymptotically dominant.
\end{theorem}

\begin{theorem}[Global barrier asymptotic]
\label{thm:global-beta-asymp}
Let \(y_*\in(\pi,2\pi)\) be the unique solution of
\[
\tan y=y,
\]
and set
\[
\gamma:=-\frac{\sin y_*}{y_*},
\qquad
C(a):=\gamma\,\frac{(1-a)^3}{1+a}.
\]
Then
\[
\beta_n(a)
=
C(a)\,(n+1)\,a^{n/2}\bigl(1+O_a(n^{-2})\bigr),
\qquad n\to\infty.
\]
In particular,
\[
...
\begin{corollary}[Eventual connectedness]
\label{cor:eventual-connect}
For every fixed \(a\in(0,1)\) and \(\varepsilon>0\), the pseudospectrum
\(\Sigma_{\varepsilon,n}\) is connected, hence simply connected, for all sufficiently
large \(n\).
\end{corollary}

\begin{corollary}[Asymptotics of the connectivity threshold]
\label{cor:threshold-asymp}
Let
\[
b(a):=\frac{|\log a|}{2}.
\]
Then the continuous inversion of the leading asymptotic
\[
\beta_n(a)\sim C(a)(n+1)e^{-b(a)n}
\]
gives
\[
N_c^{\mathrm{ev}}(a,\varepsilon)
...
-1-\frac{1}{b(a)}
W_{-1}\!\left(
-\frac{b(a)\sqrt a}{C(a)}\,\varepsilon
\right)
+O(1),
...
+
\log\frac{2C(a)}{\sqrt a\,|\log a|}
\right]
-1+O(1).
\]
Moreover, for all sufficiently small \(\varepsilon>0\),
\[
N_c^{\mathrm{first}}(a,\varepsilon)=N_c^{\mathrm{ev}}(a,\varepsilon).
\]
\end{corollary}

\begin{proposition}[Exact rank-two reduction]
\label{prop:rank-two-reduction}
Let
\[
J_n=\operatorname{tridiag}(1,0,1),
\qquad
\delta:=1-a,
\]
and define
\[
H_n(a,x):=(xI_n-\mathbf A_n(a))^{\mathsf T}(xI_n-\mathbf A_n(a)).
\]
Then
\[
H_n(a,x)
=
p_{a,x}(J_n)-\delta\,e_1e_1^{\mathsf T}+a\delta\,e_ne_n^{\mathsf T},
\]
where
\[
p_{a,x}(z)=az^2-(1+a)xz+x^2+\delta^2.
\]
If
\[
P_n(a,x,\lambda):=p_{a,x}(J_n)-\lambda I_n
\]
is invertible, set
\[
g_n(a,x,\lambda):=e_1^{\mathsf T}P_n(a,x,\lambda)^{-1}e_1
=e_n^{\mathsf T}P_n(a,x,\lambda)^{-1}e_n,
\]
\[
k_n(a,x,\lambda):=e_1^{\mathsf T}P_n(a,x,\lambda)^{-1}e_n
=e_n^{\mathsf T}P_n(a,x,\lambda)^{-1}e_1.
\]
Then
\[
\det\!\bigl(H_n(a,x)-\lambda I_n\bigr)
=
\det P_n(a,x,\lambda)\,\mathcal F_n(a,x,\lambda),
\]
with
\[
\mathcal F_n(a,x,\lambda)
=
\bigl(1-\delta\,g_n(a,x,\lambda)\bigr)
\bigl(1+a\delta\,g_n(a,x,\lambda)\bigr)
+
a\delta^2\,k_n(a,x,\lambda)^2.
\]
Hence real-axis singular-value levels are determined by the scalar equation
\[
\mathcal F_n(a,x,\lambda)=0,
\]
and local real-axis barriers satisfy the coupled system
\[
\mathcal F_n(a,x,\lambda)=0,
\qquad
\partial_x\mathcal F_n(a,x,\lambda)=0.
\]
\end{proposition}

\begin{proposition}[Exact even central barrier]
\label{prop:exact-even-central}
Let \(n=2m\), and let \(\beta_{2m}^{\mathrm{cent}}(a)\) denote the barrier in the
central gap. Then \(\beta_{2m}^{\mathrm{cent}}(a)\) is attained at \(x=0\), and if
\(q\in(a,1)\) is the unique solution of
\[
q-a=q^{2m+1}(1-aq),
\]
then
\[
\beta_{2m}^{\mathrm{cent}}(a)^2=q^{2m}(1-aq)^2.
\]
Equivalently, setting \(\delta:=1-a\),
\[
\beta_{2m}^{\mathrm{cent}}(a)^2
=
\delta^2+4a\sin^2\theta
\]
with
\[
\delta
=
2a\sin\theta\,
\frac{\cos((2m+1)\theta)}{\sin(2m\theta)}
\]
in the oscillatory branch, and
\[
\beta_{2m}^{\mathrm{cent}}(a)^2
=
\delta^2-4a\sinh^2 t
...
=
2a\sinh t\,
\frac{\cosh((2m+1)t)}{\sinh(2m t)}
\]
in the hyperbolic branch. In particular,
\[
\beta_{2m+2}^{\mathrm{cent}}(a)<\beta_{2m}^{\mathrm{cent}}(a)
\qquad(m\ge1).
\]
\end{proposition}

\begin{proposition}[Matched center/edge model]
\label{prop:matched-model}
Define
\[
C_n(a):=(1-a^2)a^{n/2},
\qquad
O_n(a):=\gamma\,\frac{(1-a)^3}{1+a}(n+1)a^{n/2},
\]
and
\[
M_n(a):=\max\{C_n(a),O_n(a)\}.
\]
Then
\[
M_{n+1}(a)<M_n(a)
\qquad\text{for every }n\ge2\text{ and }0<a<1.
\]
The model switch between the center-dominated and edge-dominated regimes
occurs when
\[
\gamma\,\frac{(1-a)^2}{(1+a)^2}(n+1)=1.
\]
Equivalently, with
\[
N:=n+1,
...
w:=(1-a)\sqrt N,
\]
the model threshold is
\[
w_N^{\mathrm{mod}}=\frac{2}{\sqrt\gamma+N^{-1/2}}.
\]
\end{proposition}

\begin{proposition}[Finite-prefix reduction]
\label{prop:finite-prefix-reduction}
For each \(n\ge2\), the violation set
\[
V_n:=\{a\in(0,1):\beta_{n+1}(a)>\beta_n(a)\}
\]
is semialgebraic. Hence any universal large-\(n\) monotonicity theorem
\[
\beta_{n+1}(a)\le\beta_n(a)
\qquad(n\ge N_\infty,\ 0<a<1)
\]
reduces the global conjecture to finitely many emptiness questions
\[
V_2,\dots,V_{N_\infty-1}.
\]
Moreover,
\[
V_2=\varnothing,
...
Equivalently,
\[
\beta_3(a)\le \beta_2(a),\qquad
\beta_4(a)\le \beta_3(a),
\qquad 0<a<1.
\]
\end{proposition}

\begin{remark}[Global monotonicity conjecture and current status]
\label{rem:global-beta-conjecture}
The natural global conjecture is
\[
\beta_{n+1}(a)\le \beta_n(a)
\qquad
(n\ge2,\ 0<a<1).
...
N_c^{\mathrm{first}}(a,\varepsilon)=N_c^{\mathrm{ev}}(a,\varepsilon)
\qquad
\text{for every }a\in(0,1),\ \varepsilon>0.
\]

The exact rank-two reduction in
Proposition~\ref{prop:rank-two-reduction}, the exact even-central formula in
Proposition~\ref{prop:exact-even-central}, the fixed-\(a\) outer-gap asymptotics in
Theorem~\ref{thm:global-beta-asymp}, and the monotone matched model in
Proposition~\ref{prop:matched-model} reduce the unresolved part of the proof to a
narrow transition strip on the scale
\[
w=(1-a)\sqrt{n+1}
\]
near the model threshold \(w_N^{\mathrm{mod}}\). Numerical experiments performed
during the investigation produced no counterexample to the global monotonicity
conjecture for \(2\le n\le55\) on dense grids in \(a\in[0.01,0.99]\), and all
observed center/edge crossovers are consistent with the monotone matched model.
In particular, the numerical evidence continues to support the universal large-\(n\)
cutoff
\[
N_\infty=2.
\]
