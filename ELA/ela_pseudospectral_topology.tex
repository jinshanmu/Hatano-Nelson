% Electronic Journal of Linear Algebra submission manuscript.
...
% ELA page dimensions from the official sample.
...
\newcommand{\Title}{Pseudospectral topology of complex tridiagonal Toeplitz
...
China (jinshanmu1129@pku.edu.cn). ORCID: 0009-0009-5346-753X.}}
\markboth{\Names}{Pseudospectral topology of complex tridiagonal Toeplitz
matrices}
\maketitle

\begin{abstract}
For \(T_n(\alpha,d,\beta)=\tridiag(\alpha,d,\beta)\), with complex
entries and \(n\geq2\), this paper classifies the connectedness of the
strict Euclidean pseudospectrum and its dependence on the matrix order.
Unitary diagonal conjugation, reversal, translation, rotation, and scaling
reduce every nonscalar family to \(A_n(a)=\tridiag(a,0,1)\), where
...
governed by one least-singular-value barrier.  For \(0<a<1\), that
barrier is \(\max\{|\alpha|,|\beta|\}\gamma_n(a)\), where
\(\gamma_{n+1}(a)<\gamma_n(a)\) for every \(n\geq2\) and
\(\gamma_n(a)\asymp_a(n+1)a^{n/2}\).  At the normal boundary \(a=1\),
the threshold is an explicit half-gap and decreases strictly at the
algebraic scale \(\pi\max\{|\alpha|,|\beta|\}/(n+1)\).  At the reducible
boundary \(a=0\), the pseudospectrum is a disk and is connected at every
order.  Thus every component is contractible, connectedness persists with
dimension, and the normal, nonnormal, and reducible regimes have distinct
transition scales.
\end{abstract}

\begin{keywords}
pseudospectrum, tridiagonal Toeplitz matrix, nonnormal matrix, least singular
value, connectedness threshold, finite-size monotonicity
\end{keywords}

\begin{AMS}
15A18, 15A60, 15B05, 65F15
\end{AMS}

\section{Introduction}\label{sec:introduction}

For \(n\geq1\), a matrix \(A\in\C^{n\times n}\), and \(\varepsilon>0\), the strict Euclidean
...
also tied to distance-to-defectivity questions.  For matrices with distinct
eigenvalues, first coalescence is characterized by a generalized saddle point
of the smallest-singular-value function, and its level determines the
spectral-norm distance to defectivity \cite{AlamEtAl2011}.  These observations
motivate a finite-dimensional topological problem that is not determined by
the spectrum alone.

The object of this paper is the full complex tridiagonal Toeplitz family
...
superdiagonal.  Although the family has three complex parameters, its
Euclidean pseudospectral topology has only two effective real parameters.
When
\[
 c=\max\{|\alpha|,|\beta|\}>0,
 \qquad a=\frac{\min\{|\alpha|,|\beta|\}}{c}\in[0,1],
\]
a diagonal unitary, possibly followed by reversal, reduces
...
\begin{equation}\label{eq:canonical-matrix}
 A_n(a)=\tridiag(a,0,1).
\end{equation}
Thus \(a=0\) is the reducible shift boundary, \(0<a<1\) is the irreducible
nonnormal regime, and \(a=1\) is the normal boundary.  Treating all three
cases gives a complete classification rather than a result for one selected
orientation or choice of phases.

The finite-size difficulty is the comparison of global barriers defined on
order-dependent spectral intervals.  The diagonal similarity identifies the
spectrum, while its condition number \(a^{-(n-1)/2}\) leaves Euclidean
least-singular-value geometry uncontrolled.  Ordinary interlacing orders
pointwise signed branches, whereas the barrier is a maximum over an
order-dependent interval and its central gap changes parity.  Toeplitz
asymptotics identify the eventual scale rather than strict inequalities at
adjacent finite orders.  The argument below combines these tools with a
strict comparison of the maximizing barrier at every consecutive order.

The first contribution is independent of Toeplitz structure.  A general
vertical-monotonicity theorem shows that, for any finite matrix with real
spectrum, contraction of the least-singular-value level sets toward the
real axis makes every pseudospectral component contractible and reduces
connectedness to the largest backward-error barrier on the convex hull of
the spectrum.  Unitary--affine reduction transfers this statement to a
matrix whose spectrum lies on any affine line.

For the nonnormal canonical regime \(0<a<1\), write
\[
 \gamma_n(a)=\max_{|x|\leq2\sqrt a\cos(\pi/(n+1))}
 s_{\min}(xI_n-A_n(a)).
\]
The central finite-size result is
\(\gamma_{n+1}(a)<\gamma_n(a)\) for every \(n\geq2\).  Explicit bounds give
\begin{equation}\label{eq:intro-barrier-order}
 \gamma_n(a)\asymp_a(n+1)a^{n/2},
\end{equation}
including the sharp polynomial prefactor.  At \(a=1\), the exact barrier is
half the largest adjacent eigenvalue gap and decreases strictly at order
\((n+1)^{-1}\).  At \(a=0\), the pseudospectrum is a disk and is connected
for every \(n\).  The classification therefore identifies a singular
change from an exponential transition scale in the nonnormal interior to an
algebraic scale at the normal boundary, followed by immediate connectedness
at the reducible boundary.
...
complete unitary--affine classification of complex tridiagonal Toeplitz
pseudospectral connectedness, together with an exact finite-size order law
in every irreducible regime and sharp transition asymptotics.  The new
finite-dimensional ingredient is the strict all-\(n\) barrier comparison,
which complements the cited spectral and asymptotic results.

The proof separates general topology and unitary--affine geometry from the
canonical nonnormal calculation.  In the latter, a pentadiagonal determinant
certificate establishes vertical monotonicity, and persymmetry turns
singular values into signed eigenvalues of a symmetric pencil.  Folded
continuants, Chebyshev divided differences, chord comparisons, and a
separate central-gap certification prove strict finite-size comparison.
Rectangular factorizations then give the sharp bounds and their logarithmic
inversion.  The longer algebraic certificates are isolated in
\crefrange{app:vertical-certificate}{app:central-certification}.  A short
physical correspondence appears briefly in \cref{sec:discussion}.
...
\end{equation}
This hypothesis is stated directly in the form needed by strict
pseudospectral sublevel sets.  For a real matrix it follows, for example,
when \(y\mapsto h_B(x,y)\) is nondecreasing on \([0,\infty)\), because
singular values are invariant under complex conjugation.
...
\begin{equation}\label{eq:abstract-barrier}
 ...
 s_{\min}(xI_n-B).
\end{equation}
The maximum exists by continuity.  It is a global pointwise backward-error
barrier: for each \(x\),
\begin{equation}\label{eq:backward-error-abstract}
 s_{\min}(xI_n-B)=
 \min\{\lVert\Delta B\rVert_2:x\in\sigma(B+\Delta B)\}.
\end{equation}
Indeed, if \((xI_n-B-\Delta B)v=0\) for a unit vector \(v\), then
\(s_{\min}(xI_n-B)\leq\lVert\Delta B\rVert_2\).  Conversely, for unit
singular vectors satisfying \((xI_n-B)v=s_{\min}(xI_n-B)u\), the rank-one
choice \(\Delta B=s_{\min}(xI_n-B)uv^*\) attains equality.
...
\begin{lemma}[Eigenvalue in every component]\label{lem:component}
...
The theorem is invariant under a Euclidean similitude of the spectral
plane: translation, rotation, and positive scaling merely translate,
rotate, and scale the pseudospectrum and its barrier.  This observation will
allow the real-axis statement to govern every matrix in
\eqref{eq:A-matrix}, whose spectrum lies on an affine line.

\section{Complex tridiagonal Toeplitz matrices and main results}
\label{sec:main-results}

We begin with the transformations that remove the inessential phases,
orientation, diagonal, and overall scale from \eqref{eq:A-matrix}.  Let
...
\begin{proposition}[Unitary--affine reduction]\label{prop:complex-reduction}
...
\(\beta=0\) and \(\alpha\ne0\), take \(S_n=J_n\) and
\(\omega=\alpha/|\alpha|\).  Equation
\eqref{eq:complex-pseudospectrum-reduction} follows
because unitary conjugation preserves singular values, while
\[
...
Thus the canonical parameter \(a\) covers both boundary cases.  Their
thresholds can be computed without the nonnormal argument.  Define
...
\begin{proof}
The scalar case is immediate.  If exactly one off-diagonal is nonzero,
\cref{prop:complex-reduction} reduces the matrix to the nilpotent Jordan
shift \(V_n\), up to a plane similitude.  By \cref{lem:component}, every
...
\(\sigma_\varepsilon(V_n)=\{z:|z|<R_{n,\varepsilon}\}\).  The plane
similitude gives the asserted disk centered at \(d\).

In part~2, \cref{prop:complex-reduction} reduces the problem to the real
symmetric matrix \(A_n(1)\), whose eigenvalues are
\(2\cos(k\pi/(n+1))\), \(1\leq k\leq n\).  Its strict pseudospectrum is the
...
\]
so the vertical contraction property is immediate.  Component
contractibility follows from \cref{thm:vertical-topology} and is preserved by
the plane similitude in \eqref{eq:complex-pseudospectrum-reduction}.
...
We now state the nonnormal theorem in its canonical variables.  Fix
...
\(a\) is fixed, its argument will be suppressed.  The explicit lower
and upper barriers are
\begin{align}
...
\(\gamma_{n+1}(a)<\gamma_n(a)\), tends to zero, and
\[
 \sigma_\varepsilon(A_n(a))\text{ is connected}
...
\[
 N_T(\alpha,\beta;\varepsilon)=
 \min\{n\geq2:\sigma_\varepsilon(T_n(\alpha,d,\beta))
 \text{ is connected}\}.
\]
The classification below shows that this minimum exists.
By \cref{prop:complex-reduction}, this value is independent of \(d\) and of
the phases of the off-diagonal entries.  Whenever \(\alpha\beta\ne0\), the
notation below uses
\[
...
\label{thm:main}
Let \(\alpha,d,\beta\in\C\), \(\varepsilon>0\), and \(n\geq2\).
\begin{enumerate}
\item Every component of \(\sigma_\varepsilon(T_n(\alpha,d,\beta))\) is
contractible, and
...
\end{equation}
\item If \(\alpha\beta\ne0\), then
\(\Theta_{n+1}(\alpha,\beta)<\Theta_n(\alpha,\beta)\) and
\(\Theta_n(\alpha,\beta)\to0\).  Hence connectedness persists after its
first occurrence.  If \(\alpha\beta=0\), then the threshold is identically
zero and the pseudospectrum is already connected at every order, so
persistence is immediate.
\item In the irreducible nonnormal case \(0<a<1\),
\begin{equation}\label{eq:general-nonnormal-critical}
 N_T(\alpha,\beta;\varepsilon)=N_c(a,\varepsilon/c)
 =\frac{2}{\log(1/a)}
...
\end{equation}
If \(\alpha\beta=0\), then \(N_T(\alpha,\beta;\varepsilon)=2\).
\end{enumerate}
\end{theorem}

\begin{proof}
For \(c>0\), apply \cref{prop:complex-reduction}.  The nonnormal assertions
follow from \cref{thm:canonical-main}, with \(\varepsilon\) replaced by
\(\varepsilon/c\); the boundary assertions follow from
\cref{prop:boundary-cases}.  In the normal case, put
\(m=N_T(\alpha,\beta;\varepsilon)\).  For sufficiently small
...
\eqref{eq:general-normal-critical}.  If \(c=0\), the scalar case applies
directly.
\end{proof}
...
\end{equation}
The strict inequality is essential: at \(\varepsilon=\gamma_n(a)\), a
maximizing real point remains excluded.
...
central gap, and \(d_{L-1}>c_L\).  Together with the upper comparison
\(c_m>d_m\), this proves \eqref{eq:central-interlace}.
\end{proof}
...
dimension, which proves the set identity in part~3.  Contractibility and the
...
The classification separates three genuinely different finite-size
mechanisms.  In the irreducible nonnormal regime, the exact threshold has
the exponential--polynomial scale \(c(n+1)a^{n/2}\), and the first connected
order grows logarithmically in \(1/\varepsilon\).  At the normal boundary,
the threshold is the largest half-gap, has scale \(\pi c/(n+1)\), and the
critical order grows as \(1/\varepsilon\).  At the reducible boundary, the
pseudospectrum is already a disk and the threshold is zero.  The two
boundaries are singular limits of the interior scale: the remainder
\(O_a(1)\) in
\eqref{eq:N-asymptotic-main} is pointwise for fixed \(0<a<1\).

This trichotomy also explains the role of strict dimension monotonicity.
For every irreducible complex tridiagonal Toeplitz family, normal or
nonnormal, the final barrier decreases at each order, so the connected
orders form one integer tail.  At reducibility the barrier collapses to zero
at every order, and persistence holds without a strict inequality.  The
statement is topological rather than merely spectral: pseudospectral
components are robust spectral clusters under an unstructured norm bound,
and \(N_T\) is the first order at which all such clusters have coalesced.
...
\(h=\tfrac12\log(t_L/t_R)\), \eqref{eq:intro-similarity} gives the familiar
right-eigenvector envelope \(e^{-h(j-1)}\) \cite{AmmariEtAl2024}; the same
exponent appears in \(a^{n/2}=e^{-hn}\).  This interpretation motivates the
three mathematical regimes within a standard nonreciprocal-chain model.

In summary, the general vertical theorem identifies the topological
mechanism, the unitary--affine reduction classifies the full complex family,
and the canonical comparison supplies the nontrivial all-order law.  The
result is an exact finite-dimensional classification of connectedness,
including the boundary levels.
...
\(\theta_{\rm ch}(x)=\theta_0\).  The classification just proved and uniqueness of
...
At \(x=x_*\) and \(s=\pm\tau_*\), where \(\tau_*=c_L\), the real pair
...
\bibliography{ela_pseudospectral_topology}
