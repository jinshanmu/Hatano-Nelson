...
\begin{hypothesis}[External input for the remaining strip]\label{hyp:standing-input}
Fix \(n\ge 2\) and \(\varepsilon>0\), and let \(a_-\) be the number given by
Proposition~\ref{prop:low-a}. Assume there exists a number
\[
\widehat a_n\in(a_-,1)
\]
...
I:=[a_-,\widehat a_n],
\]
every boundary point of \(P_{a,\varepsilon}\) is represented in the chart introduced below, namely by an equation \(\Phi_a(u,v)=0\) with \(\mathbf N_a(t(u,v),\beta(u,v))\succ 0\).
\end{enumerate}
\end{hypothesis}

\begin{remark}
Hypothesis~\ref{hyp:standing-input} is where the endpoint analysis and the global coverage of the remaining strip enter. It is not proved in this note.
\end{remark}
...
Assume Hypotheses~\ref{hyp:standing-input}(3) and \ref{hyp:monotone-m}. Then for every
\[
a_*\in I,
\]
...
\]
By Hypothesis~\ref{hyp:standing-input}(3), the origin is represented in the chart, so
\[
\Phi_{a_*}(0,0)=0.
\]
By Lemma~\ref{lem:derivative-identity},
...
After interchanging \(u\) and \(v\) if necessary, we may assume
\[
...
By Hypothesis~\ref{hyp:standing-input}, the statement is already known in the low-\(a\) regime \(0<a<a_-\) and in the near-normal regime \(\widehat a_n<a\le 1\). Thus only the compact strip
\[
I=[a_-,\widehat a_n]
\]
remains.
...
Therefore no such bad parameter exists on \(I\). Combining this with the two endpoint regimes from Hypothesis~\ref{hyp:standing-input} proves the theorem.
\end{proof}

\begin{remark}
Theorem~\ref{thm:main} is explicitly conditional: to obtain an unconditional proof one must establish Hypotheses~\ref{hyp:standing-input}, \ref{hyp:monotone-m}, and \ref{hyp:min-fold} by separate analytic arguments. The content of the theorem is componentwise rather than global: disconnected pseudospectral components may coexist, but under the stated hypotheses no hole can be born inside any one of them.
\end{remark}
