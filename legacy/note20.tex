...
\[
\mathbf v(x,y)=\bigl[v_1(x,y),\dots,v_n(x,y)\bigr]^{\mathsf T}
\]
and a normalized left singular vector
\[
\mathbf u(x,y)=\bigl[u_1(x,y),\dots,u_n(x,y)\bigr]^{\mathsf T}
\]
...
\begin{proposition}[Vertical monotonicity on the imaginary axis]\label{prop:x=0-monotonicity}
For every \(y>0\),
\[
\frac{\partial}{\partial y}\bigl(s(0,y)^2\bigr)\ge 0.
\]
Hence Conjecture~\ref{conj:vertical-monotonicity} holds for \(x=0\).
\end{proposition}
...
Let
\[
\mathbf D\coloneqq \operatorname{diag}(1,i,i^2,\dots,i^{n-1}),
\qquad
\mathbf G_y\coloneqq \mathbf D^*\,\mathbf H_0(y)\,\mathbf D.
\]
A direct computation shows that \(\mathbf G_y\) is real symmetric, with diagonal entries
\[
(\mathbf G_y)_{11}=a^2+y^2,
\qquad
(\mathbf G_y)_{jj}=1+a^2+y^2\quad (2\le j\le n-1),
\qquad
(\mathbf G_y)_{nn}=1+y^2,
\]
and nonzero off-diagonal entries
\[
(\mathbf G_y)_{j,j+1}=-(1-a)y<0,
\qquad
(\mathbf G_y)_{j,j+2}=-a<0,
\]
whenever the indices are defined. Thus \(\mathbf G_y\) is a real symmetric irreducible \(5\)-diagonal \(Z\)-matrix.

Choose \(\Lambda>\max_{1\le j\le n}(\mathbf G_y)_{jj}\). Then \(\Lambda\mathbf I_n-\mathbf G_y\) is entrywise strictly positive, so by Perron--Frobenius its largest eigenvalue is simple and has a strictly positive eigenvector. Equivalently, the smallest eigenvalue of \(\mathbf G_y\) is simple and has an eigenvector
\[
\mathbf w=\bigl[w_1,\dots,w_n\bigr]^{\mathsf T},
\qquad
w_j>0\quad (j=1,\dots,n).
\]
Since \(\mathbf H_0(y)\) and \(\mathbf G_y\) are unitarily similar, the normalized right singular vector of \(\mathbf B_{0,y}\) for \(s(0,y)\) may be written as
\[
v_j=\beta\,i^{\,j-1}w_j,
\qquad
j=1,\dots,n,
...
\]
for \(j=1,\dots,n-1\). Since every \(w_j>0\), we have
\[
\Im p_j>0
\qquad
(j=1,\dots,n).
...
The proof that \(v_1\neq 0\) is similar. Assume \(v_1=0\). Then from \(j=1\),
\[
-v_2=s\,\overline{v_n},
\qquad\text{so}\qquad
v_2=-s\,\overline{v_n}.
...
\[
\mathbf v(x)=\bigl[v_1(x),\dots,v_n(x)\bigr]^{\mathsf T}
\]
...
\]
while the other sine terms are \(\ge 0\) because every neighboring \(\delta_j(x_*)\in[0,\pi]\). Hence \(f_k'(0)<0\), contradicting minimality.

If \(\delta_k(x_*)=\pi\), then
\[
\sin(\delta_k-\theta)=\sin(\pi-\theta)>0,
\]
and again the other sine terms are \(\ge 0\). Hence \(f_k'(0)>0\), again a contradiction.
...
\begin{proof}
Fix \(j\in\{1,\dots,n\}\), and suppose that \(\Im p_j\) vanishes somewhere on \(J\). Since \(p_j\) depends continuously on \(x\), there is a first point \(x_*\in J\) at which \(p_j(x_*)\in\mathbb R\).
